    ...
    <div className="flex flex-col h-full animate-in slide-in-from-bottom-4 duration-500 fade-in bg-white dark:bg-zinc-900">
      <header className="px-6 py-4 flex items-center justify-between sticky top-0 bg-white/80 dark:bg-zinc-900/80 backdrop-blur-md z-10 border-b border-zinc-100 dark:border-zinc-800/50">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-lg font-medium">Editar Tarefa</h1>
        </div>
      ...
      <form onSubmit={handleSubmit} className="p-6 flex flex-col gap-6 flex-1 overflow-y-auto pb-24">
        <div className="space-y-2 group">
          <label className="text-sm font-medium text-zinc-700 dark:text-zinc-300">O que precisas de fazer?</label>
          <Input 
            autoFocus 
            value={title} 
            onChange={e => setTitle(e.target.value)} 
            placeholder="Ex: Rever pull request do modelo de faturação" 
            className="h-12 text-base transition-all bg-zinc-50 dark:bg-zinc-950 border-transparent hover:border-zinc-200 focus:bg-white dark:focus:bg-zinc-900 shadow-sm" 
          />
        </div>

        <div className="space-y-2 group">
          <label className="text-sm font-medium text-zinc-700 dark:text-zinc-300">Projeto / Categoria <span className="text-zinc-400 font-normal ml-1">(Opcional)</span></label>
          <Input 
            value={project} 
            onChange={e => setProject(e.target.value)} 
            placeholder="Ex: Frontend Core" 
            className="bg-zinc-50 dark:bg-zinc-950 border-transparent hover:border-zinc-200 focus:bg-white dark:focus:bg-zinc-900 shadow-sm"
          />
        </div>

        <div className="space-y-2 group flex-1">
          <label className="text-sm font-medium text-zinc-700 dark:text-zinc-300">Notas Rápidas <span className="text-zinc-400 font-normal ml-1">(Opcional)</span></label>
          <textarea 
            value={description} 
